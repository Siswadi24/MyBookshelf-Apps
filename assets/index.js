...
const createReadButton = () => {
    return createButton("fi-rr-book-alt", function (event) {
...
const createTrashButton = () => {
    return createButton("fi-rr-trash", function(event){
...
const createUnreadButton = () => {
  return createButton("fi-rr-angle-circle-left", function(event){
...
// Fungsi yang digunakan untuk mencari buku
function searchBook(bookElement) {
    const bookList = document.querySelectorAll('.detail-book');
    const conta = document.querySelectorAll('.my-container');
    for(let book of bookList){
        const judul = book.childNodes[0];
        if(!judul.innerText.toLowerCase().includes(bookElement)){
            judul.parentElement.remove();
        } else {
